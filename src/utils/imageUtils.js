...
export const applyIntensityStretching = (imageSrc, intensityMin, intensityMax) => {
  return new Promise((resolve, reject) => {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    const img = new Image();
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < data.length; i += 4) {
        // Apply intensity stretching formula: (pixel - min) * (255 / (max - min))
        data[i] = ((data[i] - intensityMin) * (255 / (intensityMax - intensityMin))); // Red
        data[i + 1] = ((data[i + 1] - intensityMin) * (255 / (intensityMax - intensityMin))); // Green
        data[i + 2] = ((data[i + 2] - intensityMin) * (255 / (intensityMax - intensityMin))); // Blue
    ...
    img.onerror = () => reject(new Error('Error loading image for intensity stretching'));
  });
};
